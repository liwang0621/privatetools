...
def label_v21_to_v22(label):
    if label == "Diary & Vlog":
        label = "Diary & VLOG"
    if label == "Selfie":
        label = "Selfies"
    if label == "School Education":
        label = "School education"
    if label == "Skincare & Makeup Product & Display":
        label = "Skincare & Makeup Product&Display"
    if label == "Dressing outfits":
        label = "Dressing Outfits"
    if label == "School life":
        label = "School Life"
    if label == "Other fashion":
        label = "Other Fashion"
    if label == "Lip-Sync":
        label = "Lip-sync"
    if label == "Traditional sports":
        label = "Traditional Sports"
    if label == "Video Games":
        label = "Video games"
    return label
...
if __name__ == '__main__':
    ...
    for item in vid_lst:
        ...
        manul_lab = label_v21_to_v22(manul_lab)
    ...
    num_w = int(len(val_lst) / 10000 + 1)#以w作为单位
    ...
    fileArr = [[]for i in range(process_num)]
    for i in range(len(file_lst)):
        fileArr[i % process_num].append(file_lst[i])
