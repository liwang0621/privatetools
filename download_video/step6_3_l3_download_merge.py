...
def check_val_by_mp4_npy(val_txt, root_path, num_process=20):
    ...
    val_map = lst_to_map(val_lst, idx=0)
    assert(len(val_lst) == len(val_map.keys()))
...
if __name__ == '__main__':
    id_label_map, label_id_map = txt_to_map("/home/daib/data/l3_tag/419L3_label_index.txt")
    assert(len(id_label_map.keys()) == len(label_id_map.keys()))
    l1l3_lst = read_txt("/home/daib/data/l3_tag/20L1_419L3.txt")
    l1l3_map = lst_to_map(l1l3_lst, idx=0)
    l3l1_map = lst_to_map(l1l3_lst, idx=1)
    # 新增数据到l2_train中
    ...
    #L2的cap_idx序号是4
    # L3的cap_idx序号是5
    dst_path = "/home/daib/data/video_cls/l1_l2_tags/unlabeled_l2_200w"
    url_path = os.path.join(dst_path, "L3_330085_vid_url_l1_l2_l3_cap.csv")
    caption_path = os.path.join(dst_path, "caption.total_L3_33w.csv")
    cap_idx = 5
    ...
    #cap_csv基本不用修改
    cap_csv = "/home/daib/data/l1_l2_tags/l2_train/caption.total_20220802-1004_320w.csv"
    val_txt = "/home/daib/data/l1_l2_tags/l2_train/l2_anno.txt.total_20220802-1004_320w.txt_fileter_opsby0.2.txt_testlimit_num200.0w.txt"
    dst_cap_csv = cap_csv + ".sub_6.3.1_opsby0.2_200w.csv"
    generate_sub_csv(val_txt, cap_csv, dst_cap_csv)


    # #第五步多进程检查训练样本是否合法, 在下载的时候已经检查过了, 有需要还可以再检查一次
    # root_path = "/home/daib/data/l3_tag/l3_0929_66w/"
