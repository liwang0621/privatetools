...
def resize(img, resize_cfg, mean):
    resize_longer_size = resize_cfg["longer_size"]
    resize_shorter_size = resize_cfg["shorter_size"]
    size_divisor = resize_cfg["size_divisor"]

    ori_longer_size = max(img.shape[0], img.shape[1])
    ori_short_size = min(img.shape[0], img.shape[1])
    resize_ratio = min(resize_shorter_size / ori_short_size, resize_longer_size / ori_longer_size)
    resize_h = int(np.round(resize_ratio * img.shape[0]))
    if resize_h % size_divisor:
        pad_h = size_divisor - resize_h % size_divisor
    else:
        pad_h = 0
    resize_w = int(np.round(resize_ratio * img.shape[1]))
    if resize_w % size_divisor:
        pad_w = size_divisor - resize_w % size_divisor
    else:
        pad_w = 0
    affine_mat = np.zeros((2, 3), dtype=np.float32)
    affine_mat[0, 0] = resize_w / float(img.shape[1])
    affine_mat[1, 1] = resize_h / float(img.shape[0])
    resize_img = cv2.warpAffine(img, affine_mat, (resize_w + pad_w, resize_h + pad_h), borderValue=mean)
    return resize_img, (affine_mat[0, 0], affine_mat[1, 1])


def prepocess(img, preprocess_cfg):
    # resize img
    resize_img, resize_scale = resize(img, preprocess_cfg["resize_cfg"], preprocess_cfg["img_norm_cfg"]["mean"])

    # preprocessing
    if preprocess_cfg["img_norm_cfg"].get("to_rgb", True):
    ...
    resize_img = (resize_img.astype(np.float32) - mean) / std
